              ...
              <ul className="flex flex-nowrap items-center gap-6">
                {links.map(({ id, url, icon }) => (
                  ...
                  >
                    <a
                      href={url}
                      className="size-22 download_tech-icon_before relative flex items-center justify-center rounded-half border-2 border-s3 bg-s1 transform-borderColor duration-500"
                    >
          ...
          <ul className="mt-24 flex justify-center max-lg:hidden">
            {logos.map(({ id, url, width, height, title }) => (
              <li key={id} className="mx-10">
                <img src={url} width={width} height={height} alt={`icon of ${title}`} />
              </li>
            ))}
          </ul>
        </div>
